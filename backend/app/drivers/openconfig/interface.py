"""
OpenConfig Interface Driver
รองรับ OpenConfig YANG models สำหรับ Interface operations
"""
...
class OpenConfigInterfaceDriver(BaseDriver):
    name = "openconfig"
    ...
    def _build_set_ipv4(self, mount: str, params: Dict[str, Any]) -> RequestSpec:
        ...
        path = (
            f"{mount}/openconfig-interfaces:interfaces/interface={ifname}"
            f"/subinterfaces/subinterface=0/openconfig-if-ip:ipv4"
            f"/addresses/address={ip}"
        )
        payload = {
            "openconfig-if-ip:address": {
                "ip": ip,
                "config": {"ip": ip, "prefix-length": int(prefix)}
            }
        }
    ...
    def _build_set_ipv6(self, mount: str, params: Dict[str, Any]) -> RequestSpec:
        ...
        path = (
            f"{mount}/openconfig-interfaces:interfaces/interface={ifname}"
            f"/subinterfaces/subinterface=0/openconfig-if-ip:ipv6"
            f"/addresses/address={ip}"
        )
        payload = {
            "openconfig-if-ip:address": {
    ...
    def _build_enable(self, mount: str, params: Dict[str, Any], enabled: bool) -> RequestSpec:
        ...
        path = f"{mount}/openconfig-interfaces:interfaces/interface={ifname}/config"
        payload = {
    ...
    def _build_show_interface(self, mount: str, params: Dict[str, Any]) -> RequestSpec:
        ...
        path = f"{mount}/openconfig-interfaces:interfaces/interface={ifname}"
        return RequestSpec(
            method="GET",
